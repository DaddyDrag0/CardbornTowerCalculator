// Cardborn Infinite Dungeon parallel simulation worker.
// Loads the exact same combat/Fabled/entry-death engine as the page, but without rendering UI.
...
const ENGINE_FILES=['app-1.txt','app-2.txt','app-3.txt','app-5.txt','app-fabled-source.txt','app-7a.txt','app-7b.txt','app-7c.txt','app-7d.txt','app-7e.txt','app-fabled-restore.txt','app-8.txt','app-9.txt','app-11.txt','app-12.txt'];
...
self.onmessage=async e=>{
  ...
  try{
    await boot();
    const result=await self.__cardbornWorkerRun(payload);
    self.postMessage({type:'result',jobId:payload.jobId,result});
  }catch(err){
    self.postMessage({type:'error',jobId:payload.jobId,error:String(err?.stack||err)});
  }
};
